...
// import { ANKI_CONNECT_DELAY_MS, VIDEO_SERVER_URL } from '../config/app.js';
...
// /**
//  * A wrapper for AnkiConnect requests that includes a standard delay.
//  */
...
//     wordWrap: true,
//     // --- MODIFICATION START ---
//     // Added double border characters
//     chars: {
...
//     table.push([
//       map.sl,
//       path.basename(map.originalPath),
//       path.basename(map.newPath),
//     ]);
//   });
...
//     spinner.setText(`${progress} Updating Note ID ${update.noteId}...`);
...
//   spinner.stop();
//   logger.success(`Update complete. Successfully updated ${chalk.green(successCount)}/${updates.length} video fields.`);
//   if (successCount < updates.length) {
//     logger.error(`${updates.length - successCount} notes failed to update. Check logs above.`);
//   }
// }
...
//       const [slData, videoFiles] = await Promise.all([
//         fetchAnkiSls(),
//         getVideoFiles(absolutePath),
//       ]);

//       if (slData.length !== videoFiles.length) {
//         logger.error(`Mismatch: Found ${chalk.yellow(slData.length)} SL values but ${chalk.yellow(videoFiles.length)} video files.`);
