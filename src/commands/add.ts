...
/**
 * Formats LaTeX expressions in a string.
 * 1. Converts inline math $...$ to \(...\) and removes spaces within the LaTeX content.
 * 2. Converts display math $$...$$ to \[...\] and removes spaces within the LaTeX content.
 * @param text The input string potentially containing LaTeX.
 * @returns The string with formatted LaTeX.
 */
function formatLaTeX(text: string): string {
    if (typeof text !== 'string') return '';

    // Convert inline math $...$ to \(...\) and remove spaces within the LaTeX
    // The regex (?<!\\)\$(.*?)(?<!\\)\$ captures content between $...$
    // that are not preceded by a backslash (to ignore escaped $).
    // The callback function (match, p1) => { ... } processes each match.
    // p1 is the captured content within the $...$.
    // p1.replace(/\s+/g, '') removes all whitespace characters from p1.
    text = text.replace(/(?<!\\)\$(.*?)(?<!\\)\$/g, (match, p1) => {
        const contentWithoutSpaces = p1.replace(/\s+/g, '');
        return `\\(${contentWithoutSpaces}\\)`;
    });

    // Convert display math $$...$$ to \[...\] and remove spaces within the LaTeX
    // Similar logic as inline math, but for $$...$$ delimiters.
    text = text.replace(/(?<!\\)\$\$(.*?)(?<!\\)\$\$/g, (match, p1) => {
        const contentWithoutSpaces = p1.replace(/\s+/g, '');
        return `\\[${contentWithoutSpaces}\\]`;
    });
