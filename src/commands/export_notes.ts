...
// /**
//  * Transforms AnkiConnect note data into the desired output format.
//  * @param notes An array of AnkiConnectNoteInfo.
//  * @returns An array of TransformedNote objects.
//  */
// function transformAnkiNotes(notes: AnkiConnectNoteInfo[]): TransformedNote[] {
//     // console.log(`Transforming ${notes.length} notes...`); // Reduced output
...
//                 // 2. Transform the fetched notes
//                 const transformedNotes = transformAnkiNotes(ankiNotes);
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
             if (error.code === 'ECONNREFUSED') {
                ...
                console.error(`HTTP Error from AnkiConnect: ${error.response.status} - ${error.response.statusText}`);
                // Fixed the typo here: error.response.data.error instead of response.data.error
                if (error.response.data?.error) {
                    console.error(`AnkiConnect Message: ${error.response.data.error}`);
                } else if (error.response.data) {
                    console.error('AnkiConnect Response Data:', error.response.data);
