...
# MAX_NOTES_PER_PART = 25                   # New constant: Maximum notes per part file
...
# # MODIFIED: Added default exclusion of 'Solution' and 'Video' field.
# def run_fetch_notes(deck: str, exclude: Optional[List[str]] = None):
#     try:
...
#         # Splitting into parts with MAX_NOTES_PER_PART (25)
...
#         else:
#             # Case: 0 to 25 notes (saved as a single file: input.json)
...
#     Notes are automatically split into files with a maximum of 25 notes per file (e.g., input-1.json, input-2.json, ...).
